...
G = matrix(c(1), ncol = p, byrow = T)
FF = matrix(c(1), byrow = T, nrow = 1)
...
for(k in 1:K){
  ...
  for(v in 1:V ) {
    ...
    if(k==1) Alpha_k[[k]][,1] = rnorm(p,0,sqrt(.0001) )  
    if(k==2) Alpha_k[[k]][,1] = rnorm(p,0,sqrt(.0001) ) 
    if(k==3) Alpha_k[[k]][,1] = rnorm(p,0,sqrt(.0001) )
    for(t in 2:t.T){
