...
G = matrix(c(1,1,0,1), ncol = p, byrow = T)
FF = matrix(c(1,0), byrow = T, nrow = 1)
...
for(k in 1:K){
  ...
  if(k==1) Alpha_k[[k]][,1] = rnorm(p,c(.5,-.1),sqrt(.0001) )  
  if(k==2) Alpha_k[[k]][,1] = rnorm(p,c(-.5,.1),sqrt(.0001) ) 
  if(k==3) Alpha_k[[k]][,1] = rnorm(p,c(0,0), sqrt(.0001))
  for(t in 2:t.T){
    if(k<3){
      Alpha_k[[k]][,t] = G%*%Alpha_k[[k]][,t-1] + rnorm(p,0,sqrt(delta) )
    }else{
      Alpha_k[[k]][,t] = Alpha_k[[k]][,t-1] + rnorm(p,0,sqrt(delta) )
    }
  }
...
pdf("~/SCIOME/Writing/Figures/Beta_truth_1.pdf")
plot(Beta_truth[[1]][,1], xlab = "Vocab term", ylab = "Beta truth", col = "black" )
dev.off()
pdf("~/SCIOME/Writing/Figures/Beta_truth_2.pdf")
plot(Beta_truth[[2]][,1], xlab = "Vocab term", ylab = "Beta truth", col = "red" )
dev.off()
pdf("~/SCIOME/Writing/Figures/Beta_truth_3.pdf")
plot(Beta_truth[[3]][,1], xlab = "Vocab term", ylab = "Beta truth", col = "blue" )
dev.off()
...
save(file = paste("~/SCIOME/Code/SynData",data_set,".RData",sep=""), t.T, D, N.D,  V, K, ny_truth, Eta_truth, Kappa_Eta_truth, Beta_truth, Kappa_Beta_truth, Alpha_truth, Z_truth, W )
