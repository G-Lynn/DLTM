...
m0 = rep(0,p)
omega = pi/2
G = matrix(c(cos(omega),sin(omega),-sin(omega),cos(omega)), ncol = p, byrow = T)
...
for(k in 1:K){
  ...
  if(k==1) Alpha_k[[k]][,1] = rnorm(p,c(.5,-.1),sqrt(C0 + delta) )  
  if(k==2) Alpha_k[[k]][,1] = rnorm(p,c(-.5,.1),sqrt(C0 + delta) ) 
  if(k==3) Alpha_k[[k]][,1] = rnorm(p,c(0,0), sqrt(C0 + delta))
  for(t in 2:t.T){
...
save(file = paste("~/SCIOME/Code/SynData",data_set,".RData",sep=""),omega, t.T, D, N.D,  V, K, ny_truth, Eta_truth, Kappa_Eta_truth, Beta_truth, Kappa_Beta_truth, Alpha_truth, Z_truth, W )
