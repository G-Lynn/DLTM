...
p = 1
a2 = .25
delta2 = a2/10 #c(.005,.00001,.001,.001) #a2/10
F_dkt = 1 #c(1,0,1,0) #1
omega = 2*pi/12
G = 1
...
C_0 = rep(.1,p) #c(.01,.0000001,.1,.1) #rep(.1,p)
m_0 = rep(0,p)

KK = c(3,10,100,1000)
a2 = seq(0.01,2,len = 10)
...
for(jj in 1:length(KK)){
  ...
  for(kk in 1:length(a2)){
    ...
    eta_1 = eta_2 = matrix(nrow = nSamples, ncol = K)
      ...
      for(i in 1:nSamples){
        for(k in 1:K){
          alpha_0 = 0 #rnorm(p,m_0,sqrt(C_0))
      ...
      Doc1 = exp(eta_1)/apply(exp(eta_1),1,sum)
      Doc2 = exp(eta_2)/apply(exp(eta_2),1,sum)

      TV_Doc[jj,kk] = mean( 1 - .5*apply(abs(Doc1-Doc2),1,sum) )
  }
}

plot(a2,TV_Doc[1,], ylim = c(0,1))
