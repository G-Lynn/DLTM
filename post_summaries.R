...
for(k in 1:K){
  Beta[[k]]=list()
  tmp = read.csv(paste(stem,"Post_Summaries/Init_",init,"/Beta_",k,".csv",sep=""),header=F,stringsAsFactors=F)
  for(t in 1:t.T){
...
save(file = paste(stem,"Reproducibility/Beta_",init,".RData",sep=""),Prob_Beta, Prob_Beta_CI)
...
for(t in 1:t.T){
  tmp = read.csv(paste(stem,"Post_Summaries/Init_",init,"/Eta_",t,".csv",sep=""),header=F,stringsAsFactors=F)
...
save(file = paste(stem,"Reproducibility/Eta_",init,".RData",sep=""),Prob_Eta)
rm(Eta,Prob_Eta,tmp)

#I still need to figure out the indexing on Alpha.  
Alpha_mean=list()
Alpha_K = read.csv(paste(stem,"Post_Summaries/Init_",init,"/Alpha_",K,".csv",sep=""), header=F,stringsAsFactors=F)
Alpha_K = Alpha_K[(burnIn*p+1):(nSamples*p),]


for(k in 1:(K-1)){
    tmp = read.csv(paste(stem,"Post_Summaries/Init_",init,"/Alpha_",k,".csv",sep=""), header=F,stringsAsFactors=F)
...
save(file = paste(stem,"Reproducibility/Alpha_",init,".RData",sep=""),Alpha_mean)
rm(Alpha_mean)
...
save(file = paste(stem,"Reproducibility/Prob_Z_",init,".RData",sep=""),P.Z.mean, CI.025, CI.975)
